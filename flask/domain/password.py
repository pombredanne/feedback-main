...
def check_new_password_validity(user, old_password, new_password):
    ...
    if not user.checkPassword(old_password):
        errors.addError('oldPassword', 'Your old password is incorrect.')
        raise errors

    if user.checkPassword(new_password):
        errors.addError('newPassword', 'You new password is the same as the old one.')
        raise errors
...
def validate_change_password_request(json):
    ...
    if 'oldPassword' not in json:
        errors.addError('oldPassword', 'Old password is missing.')
        raise errors

    if 'newPassword' not in json:
        errors.addError('newPassword', 'New password is missing.')
        raise errors
...
def validate_reset_request(request):
    if 'email' not in request.get_json():
        errors = ApiErrors()
        errors.addError('email', 'Email is missing.')
        raise errors

    if not request.get_json()['email']:
        errors = ApiErrors()
        errors.addError('email', 'This Email is empty.')
        raise errors
...
def validate_new_password_request(request):
    if 'token' not in request.get_json():
        errors = ApiErrors()
        errors.addError('token', 'Your token link is invalid.')
        raise errors

    if 'newPassword' not in request.get_json():
        errors = ApiErrors()
        errors.addError('newPassword', 'You need to enter a new password.')
        raise errors
