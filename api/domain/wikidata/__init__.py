...
def wikidata_from(entity_name):
    data = Wikidata(entity_name)
    return data.selected_info


class Wikidata:
    def __init__(self, entry, long: bool = False):
        self.entry = entry
        self.entity = self.search_entry()
        if long:
            self.info = self.get_entity_info(self.entity[1], True, True)
        else:
            self.info = self.get_entity_info(self.entity[1], True, False)
        self.selected_info = self.__select_infos()

    def __select_infos(self):
        to_select = ['id', 'label', 'description']
        selected = {}
